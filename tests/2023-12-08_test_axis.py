...
class ImageAxes(BaseModel):
    ...
    axes: list[str] = Field(description="A list of characters representing the axes of the image (for example ('c','y','x'), ('b','x','y','c'), ['b', 'z', 'x', 'y', 'c']). Each entry corresponds to the axis label of the image along the corresponding dimension. 't' is the least likely axis to appear in any image")

class ImageShapeWithAxes(BaseModel):
    """The shape (dimensionality) of an image together with the best guess for its axes"""
    shape : InputImageShape = Field(description="The shape of the image")
    axes: ImageAxes = Field(description="The best guess for the image's axes")

    @validator('axes')
    def validate_axes_length(cls, v, values):
        if 'shape' in values and len(v.axes) != len(values['shape'].image_shape):
            raise ValueError("The length of axes must match the length of the image shape")
        return v

class ImageShapesWithAxes(BaseModel):
    """A list of image shapes together with their best-guessed axes labels"""
    shapes_with_axes: list[ImageShapeWithAxes] = Field(description="A list of image shapes annotated with best guess axes labels")
...
class GuessedAxes(InputImageShapes):
    ...
    @validator('guessed_axes')
    def validate_axes_length(cls, v, values):
        if np.any(len(v.guessed_axes[i]) != len(v.shapes[i]) for i in range(len(v.guessed_axes))):
            raise ValueError("The length of the axis label string must match the number of dimensions in the image's shape")
        return(v)

async def guess_image_axes(input_image_description : InputImageShapes, role: Role = None) -> ImageShapesWithAxes:
      ...
      response = await role.aask(input_image_description, ImageShapesWithAxes)
      return(response)
