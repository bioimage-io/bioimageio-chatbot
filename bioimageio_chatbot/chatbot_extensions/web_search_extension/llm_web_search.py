...
async def search_duckduckgo(query: str, max_results: int, instant_answers: bool = True,
    ...
    async with AsyncDDGS() as ddgs:
        ...
        if answer_list:
            ...
            results = []
            async for result in ddgs.text(query, region='wt-wt', safesearch='moderate',
                                    timelimit=None, max_results=max_results):
                if get_website_content:
                    result["body"] = await get_webpage_content(result["href"])
...
async def langchain_search_duckduckgo(query: str, langchain_compressor: LangchainCompressor,
