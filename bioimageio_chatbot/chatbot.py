...
def encode_data(result):
    if result is None:
        ...
        return {k: encode_data(v) for k, v in result.items()}
    elif isinstance(result, list):
        return [encode_data(v) for v in result]
    else:
        key = str(uuid.uuid4())
        storage[key] = result
        return {
...
def decode_data(result):
    if result is None:
        return result
    elif isinstance(result, (str, int, float, bool)):
        return result
    elif isinstance(result, dict):
        if ("rtype" in result) and (result["rtype"] == "object"):
            key = result["reference_id"]
            return storage[key]
        else:
            return {k: decode_data(v) for k, v in result.items()}
    elif isinstance(result, list):
        return [decode_data(v) for v in result]
    else:
        return result
...
def create_customer_service(db_path):
    ...
    async def respond_to_user(question_with_history: QuestionWithHistory = None, role: Role = None) -> str:
        ...
        if isinstance(req, (DirectResponse, LearningResponse, CodingResponse)):
            ...
            try:
                decoded = decode_data(req.dict())
                result = await response_function(decoded)
